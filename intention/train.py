...
def train(args,opts):
    ...
    # # Force CPU to bypass Apple Silicon driver bugs
    # device = torch.device("cpu")
    # print("Forcing training on CPU for stability.")
    device = get_device()
    ...
    for epoch in range(st,args.epochs):
        ...
        for idx, (batch, label) in tqdm(enumerate(train_loader)):
            ...
            # MPS Fix: Force memory to be contiguous before calculating loss
            # loss = criterion(output.contiguous(), label.contiguous())
            # --- NUCLEAR FIX: Calculate Loss on CPU ---
            if hasattr(criterion, 'weight') and criterion.weight is not None:
                w_cpu = criterion.weight.cpu()
            else:
                w_cpu = None

            # Calculate loss on CPU
            loss = torch.nn.functional.cross_entropy(output.cpu(), label.cpu(), weight=w_cpu)
            
            # DELETE THIS LINE:
            # loss = loss.to(device)  <--- DELETE THIS! 
            
            # We leave 'loss' on the CPU. 
            # When we call loss.backward(), PyTorch will safely bridge the gap to the GPU.
    ...
    if not opts.checkpoint:
        ...
        try:
            ...
            print(f"WARNING: TensorBoard graph visualization failed: {e}")
            print('INFO: Evaluation done')
