        ...
        for vid in split:
            ...
            for ped in annotations[vid]['ped_annotations'].keys():
                for sample in annotations[vid]['ped_annotations'][ped]:
                    ...
                    # --- START FIX for missing Keypoints ---
                    if '2dkp' in sample:
                        kps = sample['2dkp']
                    else:
                        # Dynamic Shape Matching
                        # The code wants to concatenate along axis=1 (the feature dimension).
                        # So we copy the shape of bboxs, but change axis 1 to be 38 (19 joints * 2 coords).
                        
                        target_shape = list(bboxs.shape)
                        
                        # Why 38? Config says num_joints=19. Usually 2D (x,y) -> 19*2 = 38.
                        # If the model expects confidence scores (x,y,c), change this to 57.
                        target_shape[1] = 36 
                        
                        kps = np.zeros(target_shape, dtype=np.float32)

                    coords = np.concatenate((kps, bboxs), axis=1)
                    # --- END FIX ---
