...
class DSTformer(nn.Module):
    ...
    def forward(self, x, return_rep=False):   
        ...
        # # --- MPS "Apple Silicon" Crash Fix ---
        
        # # 1. "Scrub" the tensor: Force it to Float32 and ensure it's on the correct device
        # #    This fixes the "Placeholder storage" error caused by float64 inputs.
        # x = x.to(device=self.joints_embed.weight.device, dtype=torch.float32)

        # # 2. Flatten to 2D: The Mac GPU handles 2D matrix multiplication much better than 3D.
        # B, F, C = x.shape
        # x = x.reshape(B * F, C)

        # # 3. Apply the Linear Layer
        # x = self.joints_embed(x)

        # # 4. Restore original 3D shape
        # x = x.reshape(B, F, -1)
        # -------------------------------------
        # --- FINAL CORRECTED FIX (Defines BF) ---
        
        # 1. Force Float32 for Mac/MPS
        x = x.to(device=self.joints_embed.weight.device, dtype=torch.float32)
        
        # 2. Get Dimensions & Define BF (Fixes UnboundLocalError)
        B, F = x.shape[:2]
        num_joints = 19
        BF = B * F  # <--- This was missing before
        
        # 3. Force Input into (Batch, Frames, 19 Joints, Features)
        x = x.reshape(B, F, num_joints, -1)
        
        # 4. Match the Linear Layer's expectation (Pad or Slice)
        current_dim = x.shape[-1]
        wanted_dim = self.joints_embed.in_features
        
        if current_dim > wanted_dim:
            # Slice (Take first N features)
            x = x[..., :wanted_dim]
        elif current_dim < wanted_dim:
            # Pad (Add zeros to the end)
            diff = wanted_dim - current_dim
            import torch.nn.functional as F
            x = F.pad(x, (0, diff), "constant", 0)

        # 5. Flatten for Linear Layer (Standard MPS Crash Fix)
        #    Squash to 2D: (Batch*Frames*19, Wanted_Dim)
        x = x.reshape(-1, wanted_dim)
        
        # 6. Apply Linear Layer
        x = self.joints_embed(x)
        
        # 7. Restore Shape using BF
        #    (Batch*Frames, 19, Embed_Dim)
        x = x.reshape(BF, num_joints, -1)
